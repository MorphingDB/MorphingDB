...
from morphingdb_test.config import  evadb_imdb_model_path, evadb_spiece_model_path
...
class IMDBTest(PytorchAbstractClassifierFunction):
    ...
    def setup(self):
        ...
        model_urls = {
            "sst2": evadb_imdb_model_path  # noqa
        }
        ...
        def sst2():
            model = torch.load(model_urls["sst2"], weights_only=False)
            return model
    ...
    def transform(self, text):
        start = time.time()
        # sp_model_path = "./models/spiece.model.old"
        ...
        elif len(tis) > max_length:
            tis = tis[:max_length]

        # 创建attention_mask和token_type_ids
        am = [1] * len(tis_int_a)
        if len(am) < max_length:
            am += [0] * (max_length - len(am))
        elif len(am) > max_length:
            am = am[:max_length]

        ttis = [0] * len(tis_int_a)
        if len(ttis) < max_length:
            ttis += [1] * (max_length - len(ttis))
        elif len(ttis) > max_length:
            ttis = ttis[:max_length]    
        ...
        stacked_inputs = torch.stack((token_ids, attention_mask, token_type_ids, position_ids), dim=1).to(device_type)
        end = time.time()
    ...
    def forward(self, tensor) -> pd.DataFrame:
        ...
        token_ids = token_ids.to(torch.device(device)).unsqueeze(0)
        attention_mask = attention_mask.to(torch.device(device)).unsqueeze(0)
        token_type_ids = token_type_ids.to(torch.device(device)).unsqueeze(0)
        position_ids = position_ids.to(torch.device(device)).unsqueeze(0)
        end = time.time()
        self.pre_time += (end-start)
