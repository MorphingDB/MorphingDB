...
# query = cursor.query("""
#     DROP FUNCTION SST2Test;
# """).df()

query = cursor.query("""
    CREATE FUNCTION IF NOT EXISTS SST2Test
    INPUT  (comment TEXT)
    OUTPUT (labels NDARRAY FLOAT32(3))
    TYPE  Classification
    IMPL  './morphingdb_test/text_test/sst2/evadb_sst2.py';
""").df()
...
def gpu_test():
    ...
    for count in TEXT_COUNT_LIST:
        ...
        sql = """SELECT SST2Test(comment) FROM postgres_data.nlp_test limit {};""".format(count)
